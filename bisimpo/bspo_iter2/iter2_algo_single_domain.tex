...
\item[Async:] \emph{applies to $\epsilon_\tau$ only}. Two
  hyperparameters $\delta_{\rm Tj}^{(+)},\delta_{\rm Tj}^{(-)}$
  replace the single $\delta_{\rm Tj}$ of sync mode. For a trajectory
  $\tau$ with advantage $\hat A_\tau$:
  \[
  \epsilon_\tau \;=\; \dbudget(dom)\,\dbudget(g(\tau))\,
                      \delta_{\rm Tj}^{(\sign(\hat A_\tau))},
  \]
  with target $\epsilon_\tau \sim 3\!\times\!10^{-4}$ when
  $\hat A_\tau>0$, else $\sim 4\!\times\!10^{-4}$. Both sides of the
  single-trajectory clip ($s_\tau^+$ and $s_\tau^-$) share the same
  chosen $\epsilon_\tau$; the asymmetry is \emph{between
  trajectories}, driven by advantage sign.

  $\epsilon_g$ is \emph{not} sign-branched in async mode: it stays at
  the sync definition of \eqref{eq:eps-g} using the single
  $\delta_{\rm Grp}$. Consequently $\mathrm{pen}_\tau$ (which uses
  $\epsilon_{g(\tau)}$) is also not affected by the async toggle.
\end{description}
...
\]
In async mode $\delta_{\rm Tj}$ splits into a signed pair
$\delta_{\rm Tj}^{(+)}, \delta_{\rm Tj}^{(-)}$. $\delta_{\rm Grp}$
remains single-valued in both modes.
